...
@Component({
  selector: 'app-automq-tenant-view',
  templateUrl: './automq-tenant-view.component.html',
  styleUrls: ['./automq-tenant-view.component.less'],
})
export class AutomqTenantViewComponent implements OnInit, AfterViewChecked, OnDestroy {

  private static readonly TENANT_STORAGE_KEY = 'automq_selected_tenant';
  ...
  ngOnInit(): void {
    this.apiService.get('/project', '/tenant/query', { projectKey: this.projectKey })
      .subscribe(({ body }: any) => {
        ...
        const saved = localStorage.getItem(AutomqTenantViewComponent.TENANT_STORAGE_KEY);
  ...
  onTenantChange(tenantId: string) {
    ...
    localStorage.setItem(AutomqTenantViewComponent.TENANT_STORAGE_KEY, tenantId);
    this.fetchTenantView();
  }
  ...
  private drawLines() {
    ...
    this.tenantView.loadBalancers.forEach((lb: any, lbIdx: number) => {
      ...
      const dnsEl = document.getElementById(`automq-lb-dns-${lbIdx}`);
      if (!dnsEl) return;
      lb.lbConfig.routes.forEach((route: any, rIdx: number) => {
        const routeEl = document.getElementById(`automq-route-${lbIdx}-${rIdx}`);
        if (routeEl) {
          try {
