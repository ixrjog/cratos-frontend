...
@Component({
  selector: 'app-cloudflare-tenant-view',
  templateUrl: './cloudflare-tenant-view.component.html',
  styleUrls: ['./cloudflare-tenant-view.component.less'],
})
export class CloudflareTenantViewComponent implements OnInit, AfterViewChecked, OnDestroy {

  private static readonly TENANT_STORAGE_KEY = 'cloudflare_selected_tenant';

  projectKey = 'CloudFlare';
  activeTenant = '';
  tenantOptions: any[] = [];
  ...
  ngOnInit(): void {
    this.apiService.get('/project', '/tenant/query', { projectKey: this.projectKey })
      .subscribe(({ body }: any) => {
        ...
        const saved = localStorage.getItem(CloudflareTenantViewComponent.TENANT_STORAGE_KEY);
  ...
  onTenantChange(tenantId: string) {
    ...
    localStorage.setItem(CloudflareTenantViewComponent.TENANT_STORAGE_KEY, tenantId);
    this.fetchTenantView();
  }
  ...
  private drawLines() {
    ...
    this.tenantView.loadBalancers.forEach((lb: any, lbIdx: number) => {
      ...
      const dnsEl = document.getElementById(`cloudflare-lb-dns-${lbIdx}`);
      if (!dnsEl) return;
      lb.lbConfig.routes.forEach((route: any, rIdx: number) => {
        const routeEl = document.getElementById(`cloudflare-route-${lbIdx}-${rIdx}`);
        if (routeEl) {
          try {
