import pages from './zh-CN/page';
import start from './zh-CN/start';
import personalize from './zh-CN/personalize';
import authGuard from './zh-CN/auth-guard';
import footer from './zh-CN/footer';
import header from './zh-CN/header';
import login from './zh-CN/login';
import register from './zh-CN/register';
import workOrderTicket from './zh-CN/work-order-ticket';
import kubernetes from './zh-CN/kubernetes';
import webTerminal from './zh-CN/web-terminal';
import rbac from './zh-CN/rbac';
import user from './zh-CN/user';
import apiSecurityRisk from './zh-CN/api-security-risk';
import accountEntity from './zh-CN/account-entity';
import commandExec from './zh-CN/command-exec';
import artifactPublish from './zh-CN/artifact-publish';
import apolloPortalImageBuild from './zh-CN/apollo-portal-image-build';
import appContinuousDelivery from './zh-CN/app-continuous-delivery';
import userScript from './zh-CN/user-script';

export default {
  ...pages,
  ...start,
  ...personalize,
  ...authGuard,
  ...footer,
  ...header,
  ...login,
  ...register,
  ...workOrderTicket,
  ...kubernetes,
  ...webTerminal,
  ...rbac,
  ...user,
  ...apiSecurityRisk,
  ...accountEntity,
  ...commandExec,
  ...artifactPublish,
  ...apolloPortalImageBuild,
  ...appContinuousDelivery,
  ...userScript,
};
